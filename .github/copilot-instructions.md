...
1. **Never compare roles with `===`.** Use `roleEquals` / `isAdminRole` / `isManagerOrAbove` / `roleIn` from [src/lib/auth/roleCompare.ts](../src/lib/auth/roleCompare.ts). The DB contains mixed casing (`OWNER`, `owner`, `Admin`, `admin`, legacy `PM`, `FIELD_TECH`). Strict equality WILL silently demote users. See [docs/RBAC.md](../docs/RBAC.md).
2. **Every Prisma query on a tenant-scoped model MUST include `orgId` in its `where` clause.** No exceptions. See [docs/TENANT_ISOLATION.md](../docs/TENANT_ISOLATION.md) for the full model→scope-column map. Portal queries use `clientId` instead.
3. **Never trust client-provided `orgId`, `companyId`, `userId`, or `clientId`.** Always resolve from the server session via `withAuth` / `requireAuth` / `safeOrgContext` / `resolveOrgSafe`.
4. **System A RBAC is deleted.** Do not re-create `src/lib/rbac.ts` or `src/lib/permissions/legacy.ts`. All role logic lives in [src/lib/auth/rbac.ts](../src/lib/auth/rbac.ts) + [src/lib/auth/roleCompare.ts](../src/lib/auth/roleCompare.ts).
...
- Role comparison pitfall: `user.role === "admin"` misses `"ADMIN"`, `"Admin"`, `"OWNER"`, `"owner"`. ALWAYS use `isAdminRole()`.
- Tenant pitfall: `prisma.claim.findUnique({ where: { id } })` leaks cross-org. ALWAYS include `orgId`.
