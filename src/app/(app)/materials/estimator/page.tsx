...
export default function MaterialEstimatorPage() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // ── BRANDED HEADER: Logo + Company Info + Headshot ──
      y = await drawBrandedHeaderClient(pdf, branding, {
        reportType: "Material Estimate",
        reportSubtitle: `${tradeLabel}  •  ${contextLabel}`,
      });
      ...
      const filename = `${branding.companyName.replace(/\s+/g, "-")}-Estimate-${tradeLabel.replace(/\s+/g, "-")}-${Date.now()}.pdf`;
      pdf.save(filename);
    } catch (err) {
