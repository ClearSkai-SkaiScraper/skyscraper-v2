...
export interface ReportData {
  ...
  generatedBy: string;
  orgName?: string;
  /** Full claim context for comprehensive reports */
  claimContext?: {
    insuredName?: string;
    carrier?: string;
    policyNumber?: string;
...
export async function generatePDFBuffer(reportData: ReportData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      // HEADER — use org branding when available
      const headerName =
        reportData.branding?.companyName || reportData.orgName || "SkaiScraper AI Report";
      doc.fontSize(28).fillColor("#2563eb").text(headerName, { align: "center" });

      doc
        .fontSize(12)
        .fillColor("#64748b")
        .text("Powered by SkaiScraper AI Analysis", { align: "center" });

      // Company contact line
      const contactParts: string[] = [];
      if (reportData.branding?.phone) contactParts.push(reportData.branding.phone);
      if (reportData.branding?.email) contactParts.push(reportData.branding.email);
      if (reportData.branding?.website) contactParts.push(reportData.branding.website);
      if (reportData.branding?.license)
        contactParts.push(`License #${reportData.branding.license}`);
      if (contactParts.length > 0) {
        doc.fontSize(9).fillColor("#94a3b8").text(contactParts.join("  •  "), { align: "center" });
      }
      ...
      if (reportData.claimContext?.insuredName) {
        doc.text(`Insured: ${reportData.claimContext.insuredName}`);
      }
      if (reportData.claimContext?.carrier) {
        doc.text(`Insurance Carrier: ${reportData.claimContext.carrier}`);
      }
      if (reportData.claimContext?.policyNumber) {
        doc.text(`Policy Number: ${reportData.claimContext.policyNumber}`);
      }
      if (reportData.claimContext?.damageType) {
        doc.text(`Damage Type: ${reportData.claimContext.damageType}`);
      }
      if (reportData.claimContext?.dateOfLoss) {
        doc.text(
          `Date of Loss: ${new Date(reportData.claimContext.dateOfLoss).toLocaleDateString()}`
        );
      }
      if (reportData.claimContext?.adjusterName) {
        const adjParts = [reportData.claimContext.adjusterName];
        if (reportData.claimContext.adjusterPhone)
          adjParts.push(reportData.claimContext.adjusterPhone);
        if (reportData.claimContext.adjusterEmail)
          adjParts.push(reportData.claimContext.adjusterEmail);
        doc.text(`Adjuster: ${adjParts.join("  •  ")}`);
      }
      ...
      doc.fontSize(18).fillColor("#1e293b").text("🏠 Property Information");
      doc.moveDown(0.5);
      ...
      // EXECUTIVE SUMMARY
      doc.fontSize(18).fillColor("#1e293b").text("📊 Executive Summary");
      doc.moveDown(0.5);
      ...
      // DAMAGE ANALYSIS
      doc.fontSize(18).fillColor("#1e293b").text("🔍 Detailed Damage Analysis");
      doc.moveDown(0.5);
      ...
      if (damage.structuralIssues.detected) {
        doc.fontSize(14).fillColor("#dc2626").text("⚠️ Structural Issues");
        doc.moveDown(0.3);
        doc.fontSize(11).fillColor("#000000");
      ...
      // ROOF METRICS
      doc.fontSize(18).fillColor("#1e293b").text("📐 Roof Metrics");
      doc.moveDown(0.5);
      ...
      // RECOMMENDATIONS
      doc.fontSize(18).fillColor("#1e293b").text("💡 AI Recommendations");
      doc.moveDown(0.5);
      ...
      if (reportData.analysis.captionedImages.length > 0) {
        doc.addPage();
        doc.fontSize(18).fillColor("#1e293b").text("📸 Image Analysis Summary");
        doc.moveDown(1);
