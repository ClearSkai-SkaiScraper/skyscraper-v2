...
/**
 * Reusable error boundary factory for (app) route sections.
 * Usage: export default makeSectionError("dashboard", "Dashboard");
 */
export function makeSectionError(tag: string, label: string) {
  return function SectionError({
    error,
    reset,
  }: {
    ...
    useEffect(() => {
      Sentry.captureException(error, {
        tags: { component: `${tag}-error-boundary` },
